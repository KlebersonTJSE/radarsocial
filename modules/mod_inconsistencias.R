...
CAMINHO_INCONSISTENCIAS <- Sys.getenv("PASTA_INCONSISTENCIAS")

if (nzchar(CAMINHO_INCONSISTENCIAS) && !dir.exists(CAMINHO_INCONSISTENCIAS)) {
    dir.create(CAMINHO_INCONSISTENCIAS, recursive = TRUE, showWarnings = FALSE)
}
...
carregar_inconsistencias <- function() {
    
    arquivos <- list.files(
        
        path =
            CAMINHO_INCONSISTENCIAS,
...
mod_inconsistencias_ui <- function(id) {
...
mod_inconsistencias_server <- function(
    ...
    moduleServer(
        ...
        function(
            ...
            observeEvent(input$gerenciar_arquivos, {
                ...
                total_arquivos <- if (nzchar(CAMINHO_INCONSISTENCIAS) && dir.exists(CAMINHO_INCONSISTENCIAS)) {
                    length(list.files(CAMINHO_INCONSISTENCIAS))
                } else {
                    0
                }
            ...
            apagar_pasta_inconsistencias <- function() {
                inicio <- Sys.time()
                
                if (!nzchar(CAMINHO_INCONSISTENCIAS) || !dir.exists(CAMINHO_INCONSISTENCIAS)) {
                    showNotification(
                        "PASTA_INCONSISTENCIAS não está configurada ou não existe.",
                        type = "error"
                    )
                    return(invisible(NULL))
                }
                
                arquivos_atuais <- list.files(CAMINHO_INCONSISTENCIAS, full.names = TRUE)
                ...
                tryCatch({
                    dados(carregar_inconsistencias())
                    atualizar_combo_codigo()
                }, error = function(e) {
                    showNotification(
            ...
            observeEvent(input$apagar_pasta, {
                ...
                arquivos_existentes <- list.files(CAMINHO_INCONSISTENCIAS)
                ...
                showModal(modalDialog(
                    ...
                    sprintf(
                        "Tem certeza que deseja apagar os %d arquivo(s) da pasta de inconsistências? Esta ação não pode ser desfeita.",
            ...
            observeEvent(input$confirmar_apagar_pasta, {
                removeModal()
                apagar_pasta_inconsistencias()
            })
            
            # ----------------------------------------
            # UPLOAD DE ARQUIVOS PARA CAMINHO_INCONSISTENCIAS
            # ----------------------------------------
            
            # Copia os arquivos enviados para CAMINHO_INCONSISTENCIAS,
            ...
            processar_upload <- function(arquivos_upload, apagar_existentes = FALSE) {
                ...
                destinos <- file.path(CAMINHO_INCONSISTENCIAS, arquivos_upload$name)
            ...
            output$tabela <- renderDT({
                ...
                validate(
                    need(
                        nrow(df) > 0,
                        "Não existem arquivos para processamento. Utilize o botão \"Gerenciar Arquivos\" para enviar os arquivos de inconsistências/rejeitados."
                    )
                )
            ...
            # "Tabela"). Nome do arquivo: "inconsistencias-"
